    ...
    <div className={cn('w-full space-y-6 ml-11', className)}>
      ...
      <div className="space-y-4">
        {Array.from({ length: rows }).map((_, rowIndex) => (
          <div
            key={rowIndex}
            className="flex gap-4 p-4 bg-gray-100 rounded-lg overflow-hidden relative"
          >
            {/* Shimmer effect */}
    ...
    <div className={cn('w-full space-y-6 lg:space-y-8', className)}>
      ...
      <div className="flex gap-2 overflow-x-auto">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="px-6 py-3 bg-gray-100 rounded-lg overflow-hidden relative min-w-[120px]"
          >
            {/* Shimmer effect */}
            <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
...
export function UserPageSkeleton({ className }: { className?: string }) {
  return (
    <div className={cn('w-full space-y-6 lg:space-y-8', className)}>
      ...
      <div className="border border-gray-200 rounded-lg p-8 overflow-hidden relative">
        <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
          ...
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, i) => (
              ...
              >
                ...
                <div className="w-8 h-8 bg-gray-200 rounded ml-4" />
              </div>
            ))}
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, index) => (
              ...
              >
                <div className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-gradient-to-r from-transparent via-white/20 to-transparent" />
                <div className="h-3 bg-gray-200 rounded animate-pulse w-16 mb-3" />
                <div className="h-5 bg-gray-200 rounded animate-pulse w-12 mb-2" />
              </div>
            ))}
          </div>
        </div>
